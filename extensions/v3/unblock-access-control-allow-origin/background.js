...
// CORS Unblock - MV3 Version
console.log('[CORS-Unblock] Extension loaded (MV3)');

// Use declarativeNetRequest to modify headers
chrome.runtime.onInstalled.addListener(async () => {
  console.log('[CORS-Unblock] Setting up header modification rules');
  ...
  try {
    ...
    console.log('[CORS-Unblock] Rules successfully applied');
  } catch (error) {
    console.error('[CORS-Unblock] Failed to apply rules:', error);
  }
});

// Log when headers are being modified
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (changeInfo.status === 'complete' && tab.url) {
    console.log('[CORS-Unblock] Headers modified for:', tab.url);
  }
});
